...
theta_k0 = zeros(1,K);
for k = 1:K
    r_k0(k) = distancia_min + (distancia_max - distancia_min).*rand;
    theta_k0(k) = -theta_max + (theta_max - (-theta_max)).* rand;
end
...
% tk,l is the distance from scatterer l to the user k
t_kl = zeros(K,L_k);
for k = 1:K
    for l = 1:L_k
...
for k = 1:K
    ...
    a_USW_ARV_temp = [];
    for in = 1:length(y_n)
        r_n0_ARV_USW(k,in) = sqrt(r_k0(k)^2 - 2*r_k0(k)*y_n(in)*sin(theta_k0(k)) + y_n(in)^2);                       % Eq. 5

        sin_theta_n_ARV_USW(k,in) = (r_k0(k)*sin(theta_k0(k)) - y_n(in))/r_n0_ARV_USW(k,in);                      % Eq. 6
...
r_n0_ARV_USW_scatt = zeros(K,L_k, length(y_n));
sin_theta_n_ARV_USW_scatt = zeros(K,L_k, length(y_n));
...
for k = 1:K
    for l = 1:L_k
        ...
        a_USW_ARV_temp = [];
        for in = 1:length(y_n)
            r_n0_ARV_USW_scatt(k,l,in) = sqrt(r_kl(k,l)^2 - 2*r_kl(k,l)*y_n(in)*sin(theta_kl(k,l)) + y_n(in)^2);                       % Eq. 5

            sin_theta_n_ARV_USW_scatt(k,l,in) = (r_kl(k,l)*sin(theta_kl(k,l)) - y_n(in))/r_n0_ARV_USW_scatt(k,l,in);                      % Eq. 6

            for m = 1:length(MM)
                b_ARV(m,in) = exp(1i*2*pi*MM(m)*d*sin_theta_n_ARV_USW_scatt(k,l,in)/lambda);
            end

            a_USW_ARV_temp = [a_USW_ARV_temp; (exp(-1i*2*pi*r_n0_ARV_USW_scatt(k,l,in)/lambda)*b_ARV(:,in))];           % eq. 7
        end

        a_ARV_USW_scatt(:,k,l) = a_USW_ARV_temp;
    end
end
...
for k = 1:K
    for l = 1:L_k
        ...
        alpha_k_l(k,l) = (rand*40)/(t_kl(k,l)*r_kl(k,l))*exp(-1i*2*pi/lambda*t_kl(k,l) + 1i*w_kl);
    end
end
...
for k = 1:K
    for i = 1:length(y)
        r_nm_UPW(k,i) = r_k0(k) - y(i)*sin(theta_k0(k));                                           % Unnecessary
    end

    p_UPW = zeros(1,length(NN));
    for n = 1:length(NN)
        p_UPW(n) = exp(1i*2*pi*NN(n)*Gamma*d*sin(theta_k0(k))/lambda);
    end

    b_UPW = zeros(1,length(MM));
    for m = 1:length(MM)
        b_UPW(m) = exp(1i*2*pi*MM(m)*d*sin(theta_k0(k))/lambda);
    end

    a_ARV_UPW(:,k) = exp(-1i*2*pi.*r_k0(k)/lambda)*kron(p_UPW,b_UPW);                            % Eq. 4
end


% Observation of the scatterer based in the in the UPW model
r_nm_UPW_scatt = zeros(K, L_k, length(y));
a_ARV_UPW_scatt = zeros(NM, L_k, K);
for k = 1:K
    for l = 1:L_k
        for i = 1:length(y)
            r_nm_UPW_scatt(k,i) = r_kl(k,l) - y(i)*sin(theta_kl(k,l));                                           % Unnecessary
        end

        p_UPW = zeros(1,length(NN));
        for n = 1:length(NN)
            p_UPW(n) = exp(1i*2*pi*NN(n)*Gamma*d*sin(theta_kl(k,l))/lambda);
        end

        b_UPW = zeros(1,length(MM));
        for m = 1:length(MM)
            b_UPW(m) = exp(1i*2*pi*MM(m)*d*sin(theta_kl(k,l))/lambda);
        end

        a_ARV_UPW_scatt(:,k,l) = exp(-1i*2*pi.*r_kl(k,l)/lambda)*kron(p_UPW,b_UPW);                            % Eq. 4
    end
end
